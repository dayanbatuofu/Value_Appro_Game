...
import training_pinn, training_supervised, training_hybrid
...
if Supervised == True:
    ...
    loss_fn_supervised = loss_functions.initialize_intersection_HJI_supervised(supervised_dataset, Weight, alpha)

    path = 'experiment_HJI_' + opt.model + '_supervised_drone' + '/'
    root_path = os.path.join(opt.logging_root, path)
...
if Hybrid == True:
    ...
    path = 'experiment_HJI_' + opt.model + '_hybrid_drone' + '/'
    root_path = os.path.join(opt.logging_root, path)
