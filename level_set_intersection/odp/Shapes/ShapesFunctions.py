...
def IntersectionShape(grid):
    ...
    Returns:
        np.ndarray: implicit surface function of the cylinder
    """
    dim_d = grid.pts_each_dim[0]
    dim_v = grid.pts_each_dim[1]
    alpha = 1e-6

    data1 = np.zeros(grid.pts_each_dim)
    for i in range(dim_d):
        for j in range(dim_v):
            data1[:, :, i, j] = (alpha*grid.vs[0]-(grid.vs[1]-18)**2).squeeze()

    data2 = np.zeros(grid.pts_each_dim)
    for i in range(dim_d):
        for j in range(dim_v):
            data2[i, j, :, :] = (alpha*grid.vs[2]-(grid.vs[3]-18)**2).squeeze()
