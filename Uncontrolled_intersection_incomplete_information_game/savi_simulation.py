...
class Simulation:
    ...
    def write_action_choice(self):
        ...
        for i in range(len(states_1) - 1):  # inference will be behind
            x1.append(states_1[i][1])
            v1.append(states_1[i][3])
            x2.append(states_2[i][0])
            v2.append(states_2[i][2])

        'writing to csv file'
        filename = self.simulation_dir + '/' + str(self.decision_type_h) + 'traj_policy_choice' + str(x1[0]) + str(x2[0]) + '_' \
                   + str(self.env.car_par[0]['par'][0]) + str(self.env.car_par[1]['par'][0]) + '.csv'
        with open(filename, 'w', newline='') as csv_file:
            csv_writer = csv.writer(csv_file)
            csv_writer.writerow(time_stamp)
            ...
            csv_writer.writerow(policy_choice_1)
            csv_writer.writerow(policy_choice_2)
