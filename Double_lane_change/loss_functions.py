...
def initialize_intersection_HJI_EL(dataset, Weight):
    def intersection_hji(model_output, gt):
        ...
        ct_1 = (epsilon - torch.sqrt((dx_12 - dx_11) ** 2 + (dy_12 - dy_11) ** 2)).squeeze()
        ct_2 = (epsilon - torch.sqrt((dx_22 - dx_21) ** 2 + (dy_22 - dy_21) ** 2)).squeeze()
...
def initialize_intersection_HJI_pinn(dataset, Weight):
    def intersection_hji(model_output, gt):
        ...
        # calculate the collision area for aggressive-aggressive case
        threshold = torch.tensor([2.5], dtype=torch.float32).to(device)  # collision penalty threshold
        ...
        dist_diff1 = (-(torch.sqrt((dx_12 - dx_11) ** 2 + (dy_12 - dy_11) ** 2) - threshold) * 5).squeeze().reshape(-1, 1).to(device)
        ...
        # calculate the collision area lower and upper bounds
        dist_diff2 = (-(torch.sqrt((dx_22 - dx_21) ** 2 + (dy_22 - dy_21) ** 2) - threshold) * 5).squeeze().reshape(-1, 1).to(device)
...
def initialize_intersection_HJI_valuehardening(dataset, gamma, Weight):
    def intersection_hji(model_output, gt):
        ...
        # calculate the collision area lower and upper bounds
        dist_diff1 = (-(torch.sqrt((dx_12 - dx_11) ** 2 + (dy_12 - dy_11) ** 2) - threshold) * gamma).squeeze().reshape(-1, 1).to(device)
        sigmoid1 = torch.sigmoid(dist_diff1)
        loss_instant1 = beta * sigmoid1
        ...
        # calculate the collision area lower and upper bounds
        dist_diff2 = (-(torch.sqrt((dx_22 - dx_21) ** 2 + (dy_22 - dy_21) ** 2) - threshold) * gamma).squeeze().reshape(-1, 1).to(device)
        sigmoid2 = torch.sigmoid(dist_diff2)
        loss_instant2 = beta * sigmoid2
        ...
        dirichlet_2 = y2[dirichlet_mask] - 1 * source_boundary_values[:, y2.shape[1]:][dirichlet_mask]
        dirichlet = torch.cat((dirichlet_1, dirichlet_2), dim=0)
        weight_ratio = torch.abs(diff_constraint_hom).sum() * weight1 / torch.abs(dirichlet).sum()
