...
def value_function(coords, model):
...
Time = torch.linspace(0, 4, steps=5)
...
for k in range(Time.shape[0]):
    for i in range(numpoints):
        for j in range(numpoints):
            ...
            V1[k][i][j], V2[k][i][j] = value_function(coords, model)
            count += 1
            print(count)
...
if save_data:
    save_path = 'value/tanh/fft_value_hybrid_lane_tanh.mat'
    # save_path = 'value/tanh/fft_value_valuehardening_lane_tanh.mat'
