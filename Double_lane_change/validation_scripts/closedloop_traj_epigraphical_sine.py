...
def optimal_z_generation(X, t, model):
    def dichotomy1(a, b, threshold, X, t, model):
        dx_1 = np.array([[2.0 * (X[0] - 0) / (95 - 0) - 1]])
        dy_1 = np.array([[2.0 * (X[1] - 31) / (37 - 31) - 1]])
        theta_1 = np.array([[2.0 * (X[2] - (-0.15)) / (0.13 - (-0.15)) - 1]])
        v_1 = np.array([[2.0 * (X[3] - 17) / (26 - 17) - 1]])
        dx_2 = np.array([[2.0 * (X[4] - 0) / (95 - 0) - 1]])
        dy_2 = np.array([[2.0 * (X[5] - 33) / (39 - 33) - 1]])
        theta_2 = np.array([[2.0 * (X[6] - (-0.13)) / (0.15 - (-0.13)) - 1]])
        v_2 = np.array([[2.0 * (X[7] - 17) / (26 - 17) - 1]])
        label1 = torch.zeros(1, 1)

        z1 = np.array([[a]])
        X1 = np.vstack((dx_1, dy_1, theta_1, v_1, dx_2, dy_2, theta_2, v_2, label1, z1))
    ...
    def dichotomy2(a, b, threshold, X, t, model):
        ...
        z2 = np.array([[a]])
        X2 = np.vstack((dx_2, dy_2, theta_2, v_2, dx_1, dy_1, theta_1, v_1, label2, z2))
...
def value_action(X, t, model):
    ...
    z1 = X[4, :]
    dx_2 = 2.0 * (X[5, :] - 0) / (95 - 0) - 1.
    dy_2 = 2.0 * (X[6, :] - 33) / (39 - 33) - 1.
    theta_2 = 2.0 * (X[7, :] - (-0.13)) / (0.15 - (-0.13)) - 1.
    v_2 = 2.0 * (X[8, :] - 17) / (26 - 17) - 1.
    z2 = X[9, :]
    label1 = torch.zeros(1, 1)
    label2 = torch.ones(1, 1)
    ...
    epsilon = (torch.tensor([2.5], dtype=torch.float32)).to(device)  # collision ratio

    # unnormalize the state for agent 1
    dx_11 = (model_output['model_in'][:, :cut_index, 1:2] + 1) * (95 - 0) / 2 + 0
    dy_11 = (model_output['model_in'][:, :cut_index, 2:3] + 1) * (37 - 31) / 2 + 31
    theta_11 = (model_output['model_in'][:, :cut_index, 3:4] + 1) * (0.13 - (-0.15)) / 2 + (-0.15)
    v_11 = (model_output['model_in'][:, :cut_index, 4:5] + 1) * (26 - 17) / 2 + 17

    # unnormalize the state for agent 2
    dx_12 = (model_output['model_in'][:, :cut_index, 5:6] + 1) * (95 - 0) / 2 + 0
    dy_12 = (model_output['model_in'][:, :cut_index, 6:7] + 1) * (39 - 33) / 2 + 33
    theta_12 = (model_output['model_in'][:, :cut_index, 7:8] + 1) * (0.15 - (-0.13)) / 2 + (-0.13)
    v_12 = (model_output['model_in'][:, :cut_index, 8:9] + 1) * (26 - 17) / 2 + 17

    # unnormalize the state for agent 1
    dx_21 = (model_output['model_in'][:, cut_index:, 5:6] + 1) * (95 - 0) / 2 + 0
    dy_21 = (model_output['model_in'][:, cut_index:, 6:7] + 1) * (37 - 31) / 2 + 31
    theta_21 = (model_output['model_in'][:, cut_index:, 7:8] + 1) * (0.13 - (-0.15)) / 2 + (-0.15)
    v_21 = (model_output['model_in'][:, cut_index:, 8:9] + 1) * (26 - 17) / 2 + 17

    # unnormalize the state for agent 2
    dx_22 = (model_output['model_in'][:, cut_index:, 1:2] + 1) * (95 - 0) / 2 + 0
    dy_22 = (model_output['model_in'][:, cut_index:, 2:3] + 1) * (39 - 33) / 2 + 33
    theta_22 = (model_output['model_in'][:, cut_index:, 3:4] + 1) * (0.15 - (-0.13)) / 2 + (-0.13)
    v_22 = (model_output['model_in'][:, cut_index:, 4:5] + 1) * (26 - 17) / 2 + 17

    # # calculate the collision area lower and upper bounds
    ct_1 = (epsilon - torch.sqrt((dx_12 - dx_11) ** 2 + (dy_12 - dy_11) ** 2)).squeeze()  # 35
    ct_2 = (epsilon - torch.sqrt((dx_22 - dx_21) ** 2 + (dy_22 - dy_21) ** 2)).squeeze()  # 35
...
if __name__ == '__main__':
    ...
    ckpt_path = './model/sine/model_epigraphical_lane_sine.pth'
    activation = 'sine'

    # Initialize and load the model
    model = modules_adaptive.SingleBVPNet(in_features=10, out_features=1, type=activation, mode='mlp',
    ...
    if save_data:
        save_path = 'closed_loop/sine/closedloop_traj_epigraphical_lane_sine.mat'
