...
class IntersectionHJI_EL(Dataset):
    def __init__(self, numpoints, pretrain=False, tMin=0.0, tMax=3, counter_start=0, counter_end=100e3,
                 pretrain_iters=2000, num_src_samples=1000, seed=0, Theta_para=(1, 1)):
        ...
        self.R1 = 70
        self.R2 = 70
        self.theta1 = Theta_para[0]
        self.theta2 = Theta_para[1]
        self.epsilon = torch.tensor([2.5])
    ...
    def __getitem__(self, idx):
        ...
        if self.pretrain:
            ...
            coords_11 = torch.cat((torch.zeros(self.num_vio, 1).uniform_(0.17, 0.27),   # x=[53,57], y=[34,36]
                                   torch.zeros(self.num_vio, 1).uniform_(0, 0.67),
                                   torch.zeros(self.num_vio, 1).uniform_(-1, 1),
                                   torch.zeros(self.num_vio, 1).uniform_(-1, 1),
                                   torch.zeros(self.num_vio, 1).uniform_(0.17, 0.27),
                                   torch.zeros(self.num_vio, 1).uniform_(-0.67, 0),
                                   torch.zeros(self.num_vio, 1).uniform_(-1, 1),
                                   torch.zeros(self.num_vio, 1).uniform_(-1, 1)), dim=1)
            coords_12 = torch.zeros(self.n_sample, self.num_states).uniform_(-1, 1)
            coords_13 = torch.cat((torch.zeros(2000, 1).uniform_(0.17, 0.27),
                                   torch.zeros(2000, 1).uniform_(0, 0.67),
                                   torch.zeros(2000, 1).uniform_(-1, 1),
                                   torch.zeros(2000, 1).uniform_(-1, 1),
                                   torch.zeros(2000, 1).uniform_(0.17, 0.27),
                                   torch.zeros(2000, 1).uniform_(-0.67, 0),
                                   torch.zeros(2000, 1).uniform_(-1, 1),
                                   torch.zeros(2000, 1).uniform_(-1, 1)), dim=1)
            z_1 = torch.zeros(self.numpoints, 1).uniform_(-9.5e-5, 400)
            ...
            z_2 = torch.zeros(self.numpoints, 1).uniform_(-9.5e-5, 400)
            ...
            if not self.counter % self.counter_data and (self.counter + 1):
                self.coords_11 = torch.cat((torch.zeros(self.num_vio, 1).uniform_(0.17, 0.27),   # x=[53,57], y=[34,36]
                                            torch.zeros(self.num_vio, 1).uniform_(0, 0.67),
                                            torch.zeros(self.num_vio, 1).uniform_(-1, 1),
                                            torch.zeros(self.num_vio, 1).uniform_(-1, 1),
                                            torch.zeros(self.num_vio, 1).uniform_(0.17, 0.27),
                ...
                self.coords_13 = torch.cat((torch.zeros(2000, 1).uniform_(0.17, 0.27),
                                            torch.zeros(2000, 1).uniform_(0, 0.67),
                                            torch.zeros(2000, 1).uniform_(-1, 1),
                                            torch.zeros(2000, 1).uniform_(-1, 1),
                                            torch.zeros(2000, 1).uniform_(0.17, 0.27),
                                            ...
                                            torch.zeros(2000, 1).uniform_(-1, 1),
                                            torch.zeros(2000, 1).uniform_(-1, 1)), dim=1)
                self.z_1 = torch.zeros(self.numpoints, 1).uniform_(-9.5e-5, 400)
                ...
                self.z_2 = torch.zeros(self.numpoints, 1).uniform_(-9.5e-5, 400)
        ...
        dx_11 = (coords_1[:, 1:2] + 1) * (95 - 0) / 2 + 0
        dy_11 = (coords_1[:, 2:3] + 1) * (37 - 31) / 2 + 31
        dx_12 = (coords_1[:, 5:6] + 1) * (95 - 0) / 2 + 0
        dy_12 = (coords_1[:, 6:7] + 1) * (39 - 33) / 2 + 33

        dx_21 = (coords_2[:, 5:6] + 1) * (95 - 0) / 2 + 0
        dy_21 = (coords_2[:, 6:7] + 1) * (37 - 31) / 2 + 31
        dx_22 = (coords_2[:, 1:2] + 1) * (95 - 0) / 2 + 0
        dy_22 = (coords_2[:, 2:3] + 1) * (39 - 33) / 2 + 33

        gT_1 = -self.alpha * ((coords_1[:, 1:2] + 1) * (95 - 0) / 2 + 0) + \
                             ((coords_1[:, 2:3] + 1) * (37 - 31) / 2 + 31 - 37) ** 2 + \
                             100 * ((coords_1[:, 3:4] + 1) * (0.13 - (-0.15)) / 2 + (-0.15) - 0) ** 2 + \
                             ((coords_1[:, 4:5] + 1) * (26 - 17) / 2 + 17 - 18) ** 2
        zT_1 = coords_1[:, 10:]
        cT_1 = self.epsilon - torch.sqrt((dx_12 - dx_11) ** 2 + (dy_12 - dy_11) ** 2)

        gT_2 = -self.alpha * ((coords_2[:, 1:2] + 1) * (95 - 0) / 2 + 0) + \
                             ((coords_2[:, 2:3] + 1) * (39 - 33) / 2 + 33 - 33) ** 2 + \
                             100 * ((coords_2[:, 3:4] + 1) * (0.15 - (-0.13)) / 2 + (-0.13) - 0) ** 2 + \
                             ((coords_2[:, 4:5] + 1) * (26 - 17) / 2 + 17 - 18) ** 2
        zT_2 = coords_2[:, 10:]
        cT_2 = self.epsilon - torch.sqrt((dx_22 - dx_21) ** 2 + (dy_22 - dy_21) ** 2)
...
class IntersectionHJI_PINN(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # set up boundary condition
        boundary_values_1 = self.alpha * ((coords_1[:, 1:2] + 1) * (95 - 0) / 2 + 0) - \
                            ((coords_1[:, 2:3] + 1) * (37 - 31) / 2 + 31 - 37) ** 2 - \
                            100 * ((coords_1[:, 3:4] + 1) * (0.13 - (-0.15)) / 2 + (-0.15) - 0) ** 2 - \
                            ((coords_1[:, 4:5] + 1) * (26 - 17) / 2 + 17 - 18) ** 2
        boundary_values_2 = self.alpha * ((coords_2[:, 1:2] + 1) * (95 - 0) / 2 + 0) - \
                            ((coords_2[:, 2:3] + 1) * (39 - 33) / 2 + 33 - 33) ** 2 - \
                            100 * ((coords_2[:, 3:4] + 1) * (0.15 - (-0.13)) / 2 + (-0.13) - 0) ** 2 - \
                            ((coords_2[:, 4:5] + 1) * (26 - 17) / 2 + 17 - 18) ** 2
...
class IntersectionHJI_Supervised(Dataset):
    def __init__(self, Hybrid_use, seed=0):
        ...
        if not self.hybrid_use:
            data_path1 = current_dir + '/validation_scripts/train_data/data_train_lane_950.mat'
        else:
            data_path1 = current_dir + '/validation_scripts/train_data/data_train_lane_500.mat'
        train_data1 = scipy.io.loadmat(data_path1)
        self.train_data1 = train_data1

        data_path2 = current_dir + '/validation_scripts/train_data/data_train_lane_spare.mat'
    ...
    def __getitem__(self, idx):
        ...
        self.lb = torch.tensor([[0], [31], [-0.15], [17], [0], [33], [-0.13], [17]], dtype=torch.float32)
        self.ub = torch.tensor([[95], [37], [0.13], [26], [95], [39], [0.15], [26]], dtype=torch.float32)
...
class IntersectionHJI_Hybrid(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        boundary_values_1 = self.alpha * ((coords_1_hji[:, 1:2] + 1) * (95 - 0) / 2 + 0) - \
                            ...
                            100 * ((coords_1_hji[:, 3:4] + 1) * (0.13 - (-0.15)) / 2 + (-0.15) - 0) ** 2 - \
        ...
        boundary_values_2 = self.alpha * ((coords_2_hji[:, 1:2] + 1) * (95 - 0) / 2 + 0) - \
                            ...
                            100 * ((coords_2_hji[:, 3:4] + 1) * (0.15 - (-0.13)) / 2 + (-0.13) - 0) ** 2 - \
