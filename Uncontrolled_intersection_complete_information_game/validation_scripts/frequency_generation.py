...
d1_axis = torch.linspace(-1, 1, steps=numpoints)
d2_axis = torch.linspace(-1, 1, steps=numpoints)
...
for k in range(Time.shape[0]):
    for i in range(numpoints):
        for j in range(numpoints):
            coords1 = torch.stack((d1[i][j].unsqueeze(0),
                                   v1,
                                   d2[i][j].unsqueeze(0),
                                   v2), dim=1).to(device)
            coords2 = torch.stack((d2[i][j].unsqueeze(0),
                                   v2,
                                   d1[i][j].unsqueeze(0),
                                   v1), dim=1).to(device)
