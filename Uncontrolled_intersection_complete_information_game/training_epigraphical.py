...
def train(model, train_dataloader, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir, loss_fn,
          ...
          loss_schedules=None, validation_fn=None, start_epoch=0, N_choice=0):
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optim, 'min', factor=0.9, verbose=True, min_lr=1e-6,
                                                           patience=pat_para)  # (a,na), (na, a), (na, na): 5000, (a,a): 4000
    ...
    with tqdm(total=len(train_dataloader) * epochs - start_epoch) as pbar:
        ...
        for epoch in range(start_epoch, epochs):
            if not epoch % epochs_til_checkpoint and epoch:
                ...
                # if not (epoch - 50000) % 30000 and epoch and epoch >= 50000:
                #     np.savetxt(os.path.join(checkpoints_dir, 'train_losses_epoch_%04d.txt' % epoch),
                #                np.array(train_losses))
                #     np.savetxt(os.path.join(checkpoints_dir, 'hji_weight_epoch_%04d.txt' % epoch),
                #                np.array(HJI_weight))
                #     np.savetxt(os.path.join(checkpoints_dir, 'bcs_lossesA_epoch_%04d.txt' % epoch),
                #                np.array(bcs_lossesA))
                #     np.savetxt(os.path.join(checkpoints_dir, 'bcs_lossesB_epoch_%04d.txt' % epoch),
            ...
            if not total_steps % steps_til_summary:
                tqdm.write("Epoch %d, Total loss %0.3f, bcs loss_A %0.2f, bcs loss_B %0.2f, hji weight %0.2f, lr %0.6f"
                    % (epoch, train_loss, bcs_lossA, bcs_lossB, hji_weight, lr_scheduler))
